...
opt.convert_arg_line_to_args = convert_arg_line_to_args
...
def parse_args(parser):
    ...
    parser.add_argument('--load_weights_folder', type=str,
                        help='path to a pretrained model', required=True)
...
def test_simple(args):
    ...
    encoder = networks.BaseEncoder.build(model_dim=args.model_dim)
    ...
    depth_decoder = networks.Depth_Decoder_QueryTr(in_channels=args.model_dim, patch_size=args.patch_size, dim_out=args.dim_out, embedding_dim=args.model_dim, 
                                                                query_nums=args.query_nums, num_heads=4,
                                                                min_val=args.min_depth, max_val=args.max_depth)
    ...
    with torch.no_grad():
        for idx, image_path in enumerate(paths):
            ...
            disp = outputs[("disp", 0)] # we predict depth
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() == 2:
        ...
        opt = opt.parse_args([arg_filename_with_prefix])
    else:
        opt = opt.parse_args()
    # args = parse_args()
    test_simple(opt)
